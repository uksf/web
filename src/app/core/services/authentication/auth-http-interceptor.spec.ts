...
import { HttpErrorResponse, HttpRequest, HttpResponse, HttpHandlerFn, HttpEvent } from '@angular/common/http';
...
describe('authHttpInterceptor', () => {
    ...
    function runInterceptor(req: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> {
        return TestBed.runInInjectionContext(() => authHttpInterceptor(req, next));
    }

    it('passes through successful responses', () => {
        const response = new HttpResponse({ status: 200 });
        mockNext = () => of(response);

        let result: any;
        runInterceptor(dummyRequest, mockNext).subscribe({
    ...
    it('returns UksfError for network errors (status 0)', () => {
        ...
        mockNext = () => throwError(() => networkError);
    ...
    it('does not revoke on non-auth errors', () => {
        ...
        runInterceptor(dummyRequest, mockNext).subscribe({ error: () => {} });
