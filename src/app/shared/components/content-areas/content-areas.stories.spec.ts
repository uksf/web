...
test.describe('ContentAreas', () => {
    test('Default layout visual regression', async ({ page }) => {
        ...
        await expect(page.locator('.uksf-theme')).toHaveScreenshot('content-areas-default.png');
    });
    ...
    test('Full width layout visual regression', async ({ page }) => {
        ...
        await expect(page.locator('.uksf-theme')).toHaveScreenshot('content-areas-full.png');
    });
});
