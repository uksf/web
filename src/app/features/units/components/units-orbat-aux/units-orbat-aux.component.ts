...
export class UnitsOrbatAuxComponent implements OnDestroy {
    ...
    constructor(private httpClient: HttpClient, private urls: UrlService, private auth: AuthenticationService, private router: Router) {
        this.httpClient.get(`${this.urls.apiUrl}/units/chart/auxiliary`).pipe(takeUntil(this.destroy$)).subscribe({
