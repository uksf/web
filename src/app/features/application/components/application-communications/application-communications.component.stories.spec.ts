...
test.describe('ApplicationCommunications', () => {
    test('Pending visual regression', async ({ page }) => {
        ...
        await expect(page.locator('.uksf-theme')).toHaveScreenshot('application-communications-pending.png');
    });

    test('Steam visual regression', async ({ page }) => {
        await page.goto('/iframe.html?id=application-communications--steam&viewMode=story');
        await page.waitForSelector('mat-card');
        await expect(page.locator('.uksf-theme')).toHaveScreenshot('application-communications-steam.png');
    });
    ...
    test('Discord visual regression', async ({ page }) => {
        ...
        await page.waitForSelector('mat-card');
        await expect(page.locator('.uksf-theme')).toHaveScreenshot('application-communications-discord.png');
    });
});
