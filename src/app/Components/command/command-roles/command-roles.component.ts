...
@Component({
    ...
    styleUrls: ['../../../Pages/command-page/command-page.component.css', './command-roles.component.css']
})
export class CommandRolesComponent implements OnInit {
        ...
        return (control: AbstractControl): Observable<ValidationErrors> => {
            return timer(200).pipe(
                switchMap(() => {
                    ...
                    return this.httpClient.post(`${this.urls.apiUrl}/roles/${type}/${control.value}`, {}).pipe(
                        map(response => (response ? { roleTaken: true } : null))
                    );
                })
            )
        }
    ...
    validateInlineRole(role, type): Observable<boolean> {
        return timer(200).pipe(
            switchMap(() => {
                if (role.name === null) { return of(false); }
                return this.httpClient.post(`${this.urls.apiUrl}/roles/${type}/${role.name}`, role).pipe(
    ...
    addRole(type) {
        ...
        this.httpClient.put(`${this.urls.apiUrl}/roles`, formString, {
    ...
    editRole(name) {
        ...
        if (role) {
            this.httpClient.patch(`${this.urls.apiUrl}/roles`, role, {
                headers: new HttpHeaders({
                    'Content-Type': 'application/json'
                })
            }).subscribe(response => {
    ...
    onMove(event: CdkDragDrop<string[]>) {
        ...
        if (before === JSON.stringify(this.unitRoles)) { return; }
